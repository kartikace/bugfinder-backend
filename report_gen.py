...
def generate_pdf_report(results, target_url, output_path):
    ...
    total_bugs = sum(len(v) for v in results.get('vulnerabilities', {}).values())
    
    meta_data = [
        ['Target URL', target_url],
        ['Scan Date', results.get('scan_time', datetime.utcnow().isoformat())[:19].replace('T', ' ')],
    ...
    high_count = sum(1 for v in vuln_data.values() for i in v if i.get('severity') == 'HIGH')
    med_count = sum(1 for v in vuln_data.values() for i in v if i.get('severity') == 'MEDIUM')
    low_count = sum(1 for v in vuln_data.values() for i in v if i.get('severity') == 'LOW')
    ...
    story.append(Paragraph(
        f"Report generated by BugFinder AI • {datetime.utcnow().strftime('%Y-%m-%d %H:%M')} UTC • "
